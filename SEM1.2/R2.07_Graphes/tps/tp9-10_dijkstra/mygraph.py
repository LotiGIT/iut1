class Graphe(object):
    ...
    def add_arete(self, arete, poids):
        """Ajoute une arête valuée au graphe.
        L'arête est de type set, tuple ou list; entre deux sommets, il peut y avoir plus
        d'une arête (multi-graphe).
        """
        if isinstance(arete, (set, tuple, list)) and len(arete) == 2:
            sommet1, sommet2 = arete
            self._graphe_dict.setdefault(sommet1, {}).update({sommet2: poids})

    def get_poids_arete(self, sommet1, sommet2):
        """Retourne le poids de l'arête entre deux sommets."""
        return self._graphe_dict.get(sommet1, {}).get(sommet2, None)

    def __list_aretes(self):
        """Methode privée pour récupérer les arêtes.
        Une arête est un ensemble (set) avec un (boucle) ou deux sommets.
        """
        all_edges = set()

        for sommet, voisins in self._graphe_dict.items():
            for voisin, poids in voisins.items():
                edge = set({sommet, voisin, poids})
                all_edges.add(edge)
    ...
    def trouve_chaine(self, sommet_dep, sommet_arr, chain=None):
        """Trouver un chemin élémentaire de sommet_dep à sommet_arr dans le graphe."""
